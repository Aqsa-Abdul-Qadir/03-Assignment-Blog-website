import { getAllBlogs } from '@/sanity/lib/sanityQueries'; // Adjust the import path accordingly
...
const BlogPage = async () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-8 text-charcoal-gray">
        Explore Our Blogs
      </h2>
        ...
        {blogs.map((blog: Blog) => (
          <div key={blog.slug.current} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-all">
            ...
            <Link href={`/blog/${blog.slug.current}`}>
              <button className="inline-block bg-electric-green text-midnight-blue py-1 px-4 mt-2 rounded-full text-sm font-semibold hover:bg-green-400 transition duration-300">
                Read More
              </button>
            </Link>
          </div>
